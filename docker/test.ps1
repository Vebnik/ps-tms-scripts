...
try {
  ...
  docker exec -it $testContainer /usr/bin/bash -c "cd /ps-tms-selenium-tests && mvn install -T 16 -Dtest=com.sonoma.pstms.selenium.LoginTest" | Out-File -FilePath ./logs/LoginTest.log
  ...
  docker exec -it $testContainer /usr/bin/bash -c "cd /ps-tms-selenium-tests && mvn install -T 16" | Out-File -FilePath ./logs/Test.log
  Start-Sleep -Seconds 0.25
